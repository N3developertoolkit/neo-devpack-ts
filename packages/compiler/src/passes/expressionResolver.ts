...
// function generateCallOps(tree: CallExpressionTree, scope: Scope): E.Either<ParseError, readonly Operation[]> {
//     return pipe(
//         tree.expression,
//         resolveTypeOrSymbol(scope),
//         E.fromOption(() => makeParseError(tree.node)(`failed to resolve type or symbol`)),
//         E.chain(cto => pipe(cto.parseCall, E.fromNullable(makeParseError(cto.node)(`resolved object does not support calls`)))),
//         E.chain(parseCall => parseCall(scope)(tree.node)),
//     )
// }

// function generateConstructorOps(tree: ConstructorExpressionTree, scope: Scope): E.Either<ParseError, readonly Operation[]> {
//     return pipe(
//         tree.expression,
//         resolveTypeOrSymbol(scope),
//         E.fromOption(() => makeParseError(tree.node)(`failed to resolve type or symbol`)),
//         E.chain(cto => pipe(cto.parseConstructor, E.fromNullable(makeParseError(cto.node)(`resolved object does not support new`)))),
//         E.chain(parseConstructor => parseConstructor(scope)(tree.node)),
//     )
// }

// function generateElementAccessOps(tree: ElementAccessExpressionTree, scope: Scope): E.Either<ParseError, readonly Operation[]> {
//     return E.left(makeParseError(tree.node)(`generateElementAccessOps not implemented}`));
// }

// function generatePropertyAccessOps(tree: PropertyAccessExpressionTree, scope: Scope): E.Either<ParseError, readonly Operation[]> {

//     const q = pipe(
//         tree.expression,
//         resolveTypeOrSymbol(scope),
//         E.fromOption(() => makeParseError(tree.node)(`failed to resolve type or symbol`)),
//         E.chain(cto => pipe(cto.getProperty, E.fromNullable(makeParseError(cto.node)(`resolved object does not support getProperty`)))),
//         E.chain(getProperty => {
//             return pipe(
//                 getProperty(tree.property),
//                 E.fromOption(() => makeParseError(tree.node)(`failed to resolve ${tree.property.getName()} property`)),
//             )
//         })
...
//     //     resolveTree(scope),
//     //     E.chain(cto => {
//     //         cto.getProperty(tree.property)
//     //     })
//     // )
//     return E.left(makeParseError(tree.node)(`generatePropertyAccessOps not implemented}`));
// }
