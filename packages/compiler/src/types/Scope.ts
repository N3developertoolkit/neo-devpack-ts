...
import type { CompileTimeObject } from "./CompileTimeObject";

type SymboledCTO = CompileTimeObject & { readonly symbol: tsm.Symbol }
...
const $createScope = (parentScope: O.Option<Scope>) =>
    (defs: readonly SymboledCTO[], types: readonly SymboledCTO[] = []): E.Either<string, Scope> => {
        return pipe(
            E.Do,
...
export const updateScope =
    (scope: Scope) =>
        (symbols?: SymboledCTO | readonly SymboledCTO[], types?: SymboledCTO | readonly SymboledCTO[]): E.Either<string, Scope> => {
            symbols = symbols
                ? isArray(symbols)
...
export const resolve =
    (scope: Scope) =>
        (symbol: tsm.Symbol): O.Option<SymboledCTO> => {
            return pipe(
...
export const resolveName =
    (scope: Scope) =>
        (name: string): O.Option<SymboledCTO> => {
            return pipe(
                scope.symbols,
                findFirst(name),
                O.alt(() => pipe(
