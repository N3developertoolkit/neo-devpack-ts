...
import 'mocha';
import { bigIntToByteArray } from '../src/utils'
// import { convertInt } from '../src/convert';

describe('convertInt', () => {
    // const noOperandTests = [
    //     { value: -1n, expected: sc.OpCode.PUSHM1 },
    //     { value: 0n, expected: sc.OpCode.PUSH0 },
    //     { value: 1n, expected: sc.OpCode.PUSH1 },
    //     { value: 2n, expected: sc.OpCode.PUSH2 },
    //     { value: 3n, expected: sc.OpCode.PUSH3 },
    //     { value: 4n, expected: sc.OpCode.PUSH4 },
    //     { value: 5n, expected: sc.OpCode.PUSH5 },
    //     { value: 6n, expected: sc.OpCode.PUSH6 },
    //     { value: 7n, expected: sc.OpCode.PUSH7 },
    //     { value: 8n, expected: sc.OpCode.PUSH8 },
    //     { value: 9n, expected: sc.OpCode.PUSH9 },
    //     { value: 10n, expected: sc.OpCode.PUSH10 },
    //     { value: 11n, expected: sc.OpCode.PUSH11 },
    //     { value: 12n, expected: sc.OpCode.PUSH12 },
    //     { value: 13n, expected: sc.OpCode.PUSH13 },
    //     { value: 14n, expected: sc.OpCode.PUSH14 },
    //     { value: 15n, expected: sc.OpCode.PUSH15 },
    //     { value: 16n, expected: sc.OpCode.PUSH16 },
    // ];

    // noOperandTests.forEach(({ value, expected }) => {
    //     it(`convertInt ${value}`, () => {
    //         const ins = convertInt(value);
    //         expect(ins.opCode).to.equal(expected);
    //         expect(ins.operand).to.be.undefined;
    //     })
    // });

    // function testConvertInt(value: bigint, expected: string) {
    //     it(`convertInt ${value}`, () => {
    //         const [operand, opcode] = getExpected(expected);
    //         const ins = convertInt(value);
    //         expect(ins.opCode).to.equal(opcode);
    //         expect(ins.operand).not.to.be.undefined;
    //         expect(operand.compare(ins.operand!)).to.equal(0);
    //     })

    //     function getExpected(value: string): [Buffer, sc.OpCode] {
    //         const buf = Buffer.from(expected, 'hex');
    //         if (buf.length === 1) { return [buf, sc.OpCode.PUSHINT8]; }
    //         if (buf.length === 2) { return [buf, sc.OpCode.PUSHINT16]; }
    //         if (buf.length <= 4) { return [ensureLength(buf, 4), sc.OpCode.PUSHINT32] }
    //         if (buf.length <= 8) { return [ensureLength(buf, 8), sc.OpCode.PUSHINT64] }
    //         if (buf.length <= 16) { return [ensureLength(buf, 16), sc.OpCode.PUSHINT128] }
    //         if (buf.length <= 32) { return [ensureLength(buf, 32), sc.OpCode.PUSHINT256] }
    //         throw new Error(`invalid expected ${value}`);
    //     }

    //     function ensureLength(value: Buffer, length: number) {
    //         if (value.length === length) { return value; }
    //         if (length < value.length) throw new Error();
    //         const padding = new Array<`number>(length - value.length).fill(0);
    //         return Buffer.from([...value, ...padding]);
    //     }
    // }
